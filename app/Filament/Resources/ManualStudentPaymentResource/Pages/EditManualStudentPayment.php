...
class EditManualStudentPayment extends EditRecord
{
    ...
    protected function mutateFormDataBeforeSave(array $data): array
    {
        if (filled($data['rr_student_payment_id'] ?? null)) {
            ...
            if (ManualStudentPaymentResource::hasPendingManualPayment($rrPayment->id, $this->record->id)) {
                throw \Illuminate\Validation\ValidationException::withMessages([
                    'rr_student_payment_id' => 'Item pembayaran ini sudah memiliki Pembayaran Manual lain yang menunggu/tervalidasi.',
                ]);
            }
